...
<article class="page-container">
    <form  class="form form-horizontal" id="form-change-password">
        ...
        <div class="row cl">
            ...
            <div class="formControls col-xs-8 col-sm-9"> {{$model->username}} </div>
        </div>
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-3"><span class="c-red">*</span>新密码：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="password" class="input-text" autocomplete="off"  name="newpassword" id="newpassword">
            </div>
        </div>
        <div class="row cl">
            <div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-3">
                <button type="button" class="btn btn-primary radius"  id="button" >&nbsp;&nbsp;保存&nbsp;&nbsp;</button>
            </div>
        </div>
    </form>
</article>
...
<script type="text/javascript">
    $("#button").click(function () {
        $.ajax({
            type: "POST",   //提交的方法
            url:"/NCMS-FACE/public/update_password", //提交的地址
            data:$('#form-change-password').serialize(),// 序列化表单值
            async: false,
            error: function(request) {  //失败的话
                layer.alert("密码和用户名不可为空!");
            },

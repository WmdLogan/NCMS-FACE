...
<head>
    ...
    <![endif]-->
    <title>爱唱消息内容关键字配置</title>
</head>
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 配置管理 <span class="c-gray en">&gt;</span> 爱唱消息内容关键字配置 </nav>
<div class="page-container">
    @if($auth == 1)
    <div class="cl pd-5 bg-1 bk-gray mt-20">
	<span class="l">
	<a href="javascript:;" onclick="member_add('配置爱唱消息内容关键字','/NCMS-FACE/public/keyword_aichang_add','600','270')"class="btn btn-primary radius"><i class="Hui-iconfont">&#xe600;</i> 配置爱唱消息内容关键字</a>
        ...
        <table class="table table-border table-bordered table-hover table-bg table-sort">
            ...
            @foreach($model as $val)
                <tr class="text-c">
                    ...
                    @if($auth == 1)
                    <td  class="td-manage">
                        <a title="编辑" href="javascript:" onclick="member_edit('编辑','/NCMS-FACE/public/keyword_aichang_update?id={{$val -> keyword_id}}','','600','270')" class="ml-5" style="text-decoration:none"><i  class="Hui-iconfont">&#xe6df;</i></a>
...
<script type="text/javascript">
    ...
    function member_del(obj,id){
        layer.confirm( '确认要删除吗？',function(index){
            $.post('/NCMS-FACE/public/keyword_aichang_delete',{
                    'id':id,
                    '_token':"{{csrf_token()}}",
                },
                function (data) {
    ...
    function delKeys(obj,id)
    {
        ...
        layer.confirm('您确定要删除我们吗？', function() {
            $.post("/NCMS-FACE/public/keyword_aichang_deletes", {
                "_token": "{{ csrf_token() }}",
                "keys": items
            }, function(data) {
                if (data.status == 0) {
